...
exports.addToCart = async (req, res) => {
    ...
    if(dataCart == 0){
        ...
        const updateCart = await Cart.updateOne ({id: dataCart._id}, dataCart)
        res.send(JSON.stringify({"status": 200, "error": null, "response": "Success add to cart"}))
    }
}
...
exports.checkOut = async (req, res) => {
    ...
    if(dataCart <= 0) {
        ...
        const hapusCart = await Cart.deleteMany({userCart : userCart})
        res.send(JSON.stringify({"status": 200, "error": null, "response": "Transaksi Berhasil"}))
    }
}
